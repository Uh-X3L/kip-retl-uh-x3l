...
def log_agent_operation(agent_name: str, operation_type: str, details: Dict[str, Any], 
                       status: str = "started", duration: float = None):
    """
    Log an agent operation for compatibility.
    ...
    if _global_logger:
        if status == "started":
            operation_id = _global_logger.start_agent_operation(
                agent_name=agent_name,
                agent_type="unknown",
                operation_type=operation_type,
                inputs=details
            ...
            # This is a simplified version - in real usage, you'd track operation_id
            _global_logger.add_step_log(
                step_name=f"{agent_name} - {operation_type}",
                details={
                    "status": status,
                    "duration": duration,
                    "details": details
